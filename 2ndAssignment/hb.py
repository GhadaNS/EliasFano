...
class hb:
    ...
    def ComputeAlignmentScore(self, x, y):
        ...
        for i in range(1, len(x) + 1):
            ...
            for j in range(1,
                           len(y) + 1):  # Calculating the current line from top and diagonal previous cells and left cell from L
                L[j] = max(L[j - 1] + self.g, K[j] + self.g,
                           K[j - 1] + self.compare(x[i - 1], y[j - 1]))  # Based on the principle of optimality
        return L

    def Hirschberg(self, x, y, t):  # when the sequences are characters
        if len(x) == 0:  # If x is empty, y is aligned with "-"
            ...
            Sr = self.ComputeAlignmentScore(x[i:len(x)][::-1], y[
                                                               ::-1])  # Computing the AS of the reversed second half of x with  reversed y
            ...
            for j in J:  # To slice y with all possible solutions
                if t:
                    print(i, ",", j)
                WWl, ZZl = self.Hirschberg(x[0:i], y[0:j], t)
                WWr, ZZr = self.Hirschberg(x[i:len(x)], y[j:len(y)], t)
    ...
    def Hirschberg_lines(self, x, y, t):  # when sequences are lines, same as first one but some changes to deal with it as a list
        if len(x) == 0:  # If x is empty, y is aligned with "-"
            print("x", x)
            WW = ['-' * len([y])]
            ZZ = [y]
            ...
            for j in J:  # To slice y with all possible solutions
                if t:
                    print(i, ",", j)
                WWl, ZZl = self.Hirschberg_lines(x[0:i], y[0:j], t)  # left part
                WWr, ZZr = self.Hirschberg_lines(x[i:len(x)], y[j:len(y)], t)  # right part
                print(WWl, WWr, "\n", ZZl, ZZr)
                for z, c in zip(WWl, ZZl):  # Just to make all possible permutations of the outcome
                    for k, e in zip(WWr, ZZr):
